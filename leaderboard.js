...
async function loadLeaderboardData() {
    ...
    try {
        ...
        console.log(`👥 프로필 처리된 사용자: ${userProfiles.size}명`);
        ...
        console.log(`💰 포인트 데이터가 있는 사용자: ${userPoints.size}명`);

        // 🆕 포인트 데이터가 없는 사용자 확인
        const usersWithoutPoints = [];
        userProfiles.forEach((profile, uid) => {
            if (!userPoints.has(uid)) {
                usersWithoutPoints.push(profile.nickname);
            }
        });
        
        // 🆕 포인트 데이터가 없는 사용자들을 위한 자동 생성 (옵션)
        if (usersWithoutPoints.length > 0) {
            console.log(`🔧 ${usersWithoutPoints.length}명의 포인트 데이터 자동 생성 중...`);
            
            // 배치로 포인트 데이터 생성 (너무 많으면 시간이 걸릴 수 있음)
            const batch = window.firebase.writeBatch ? window.firebase.writeBatch(db) : null;
            let createCount = 0;
            
            for (const [uid, profile] of userProfiles) {
                if (!userPoints.has(uid) && createCount < 50) { // 최대 50개씩 생성
                    ...
                    if (batch) {
                        ...
                        try {
                            ...
                            console.error(`포인트 생성 실패 (${uid}):`, error);
                        }
                    }
                    createCount++;
                    userPoints.set(uid, 0); // 메모리에도 추가
                }
            }
            
            // 배치 커밋
            if (batch && createCount > 0) {
                try {
                    await batch.commit();
                    console.log(`✅ ${createCount}개의 포인트 데이터 생성 완료`);
                } catch (error) {
                    console.error('배치 포인트 생성 실패:', error);
                }
            }
        }
        ...
        // 6. 최종 사용자 데이터 생성 및 정렬 (🆕 모든 프로필 포함)
        leaderboardUsers = Array.from(userProfiles.values()).map(profile => {
            ...
            const points = userPoints.get(profile.uid) || 0; // 🆕 포인트 없으면 0으로 설정
        ...
        console.log(`📋 리더보드 포함 사용자: ${leaderboardUsers.length}명`);
        
        // 🆕 통계 요약
        const hasPoints = leaderboardUsers.filter(u => u.points > 0).length;
        const hasVotes = leaderboardUsers.filter(u => u.totalVotes > 0).length;
        console.log(`📊 통계: 포인트 보유 ${hasPoints}명, 투표 참여 ${hasVotes}명`);
